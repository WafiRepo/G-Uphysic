...
public class DataBuffer implements Serializable {
    ...
    public String getDataAsJsonString() {
        if (buffer != null && !buffer.isEmpty()) {
            Gson gson = new Gson();
            return gson.toJson(Collections.singletonMap(name, buffer)); // Wrap in an object with buffer name as key
        }
        return "{}"; // JSON kosong jika buffer kosong
    }
    ...
    // Tambahkan metode getDataAsList untuk mengembalikan buffer sebagai List<Double>
    public List<Double> getDataAsList() {
        return new ArrayList<>(buffer); // Mengembalikan salinan dari buffer sebagai List
    }
