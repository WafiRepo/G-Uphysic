...
public class InteractiveGraphView extends RelativeLayout implements GraphView.PointInfo {
    ...
    private LinearLayout viewDrawingPad;
...
//    }
    ...
    public class Marker {
        ...
        // Getter methods untuk screenshot
        ...
        public float getDataZ() { return dataZ; }
    }
...
//        View v = toolbar.getMenu().findItem(R.id.graph_tools_draw).getActionView();
//        if (v != null) {
...
//        }
    ...
    // Method untuk mengakses data pick points untuk screenshot
    public Marker[] getMarkers() {
        return marker;
    }

    public MarkerOverlayView getMarkerOverlayView() {
        return markerOverlayView;
    }

    // Method untuk menggambar pick points ke canvas screenshot
    public void drawPickPointsToCanvas(Canvas canvas, int screenViewX, int screenViewY) {
        try {
            for (int i = 0; i < marker.length; i++) {
                if (marker[i].active) {
                    // Hitung posisi relatif terhadap screenView
                    int[] gvLocation = new int[2];
                    this.getLocationOnScreen(gvLocation);
                    float x = marker[i].viewX + (gvLocation[0] - screenViewX);
                    float y = marker[i].viewY + (gvLocation[1] - screenViewY);

                    // Gambar label info nilai pick point saja
                    Paint textPaint = new Paint();
                    textPaint.setColor(0xFFFFFFFF); // Warna putih
                    textPaint.setTextSize(32); // Ukuran teks
                    textPaint.setAntiAlias(true);
                    textPaint.setStyle(Paint.Style.FILL);
                    textPaint.setShadowLayer(4, 2, 2, 0xFF000000); // Bayangan hitam agar terbaca

                    // Format nilai
                    String label = String.format("x: %.2f\ny: %.2f", marker[i].dataX, marker[i].dataY);
                    if (!Float.isNaN(marker[i].dataZ)) {
                        label += String.format("\nz: %.2f", marker[i].dataZ);
                    }

                    // Gambar teks di samping posisi marker
                    float textX = x + 30;
                    float textY = y - 10;
                    for (String line : label.split("\\n")) {
                        canvas.drawText(line, textX, textY, textPaint);
                        textY += textPaint.getTextSize() + 4; // Spasi antar baris
                    }
                }
            }
        } catch (Exception e) {
    ...
    // Method untuk mendapatkan data pick points dalam format yang dapat digunakan untuk screenshot
    public static class PickPointData {
        public final float viewX, viewY;
        public final float dataX, dataY, dataZ;
        public final boolean active;
        
        public PickPointData(float viewX, float viewY, float dataX, float dataY, float dataZ, boolean active) {
            this.viewX = viewX;
            this.viewY = viewY;
            this.dataX = dataX;
            this.dataY = dataY;
    ...
    public PickPointData[] getPickPointsData() {
        PickPointData[] data = new PickPointData[marker.length];
        for (int i = 0; i < marker.length; i++) {
            data[i] = new PickPointData(
                marker[i].viewX, 
                marker[i].viewY, 
                marker[i].dataX, 
                marker[i].dataY, 
